...
class GraphicalView(QtGui.QGraphicsView):
    ...
    def __init__(self, modelRoot, parent, border,
                 layoutPt, createdItem, minmaxratio):
        ...
        self.cmptStackorder = [
            i for i in self.stackOrder if isinstance(
                i, ComptItem)]
        self.viewBaseType = " "
        self.iconScale = 1
        self.arrowsize = 2
    ...
    def resolveCompartmentInteriorAndBoundary(self, item, position):
        ...
        return COMPARTMENT_INTERIOR if bound.contains(
            item.mapFromScene(position)) else COMPARTMENT_BOUNDARY

    def resetState(self):
        self.state = {
            "press": {
                "mode": INVALID, "item": None, "sign": None, "pos": None}, "move": {
                "happened": False}, "release": {
                "mode": INVALID, "item": None, "sign": None}}
        ...
        for item in items:
            if hasattr(item, "name"):
                ...
                if item.name == COMPARTMENT:
                    solution = (
                        item, self.resolveCompartmentInteriorAndBoundary(
                            item, position))
    ...
    def editorMousePressEvent(self, event):
        if event.buttons() == QtCore.Qt.LeftButton:
            ...
            if len(comptList) > 1:
                ...
                popupmenu.addAction(
                    "LinearLayout", lambda: handleCollisions(
                        comptList, moveX, self.layoutPt))
                popupmenu.addAction(
                    "VerticalLayout", lambda: handleCollisions(
                        comptList, moveMin, self.layoutPt))
    ...
    def editorMouseMoveEvent(self, event):
        ...
        if self.move:
            ...
            for item in self.selectedItems:
                if isinstance(item, KineticsDisplayItem) and not isinstance(
                        item, ComptItem) and not isinstance(item, CplxItem):
        ...
        if itemType == CONNECTOR:
            ...
            if actionType == "move":
                ...
                if isinstance(item.parent(), KineticsDisplayItem):
                    ...
                    if moose.exists(itemPath):
                        ...
                        if modelAnno.modeltype == "kkit":
                            x = ((self.mapToScene(event.pos()).x()) + (self.minmaxratioDict[
                                 'xmin'] * self.minmaxratioDict['xratio'])) / self.minmaxratioDict['xratio']
                            y = (1.0 - self.mapToScene(event.pos()).y() + (self.minmaxratioDict[
                                 'ymin'] * self.minmaxratioDict['yratio'])) / self.minmaxratioDict['yratio']
                            anno.x = x
                            anno.y = y
                ...
                # if not isinstance(item.parent(),FuncItem) and not
                # isinstance(item.parent(),CplxItem):
        ...
        if itemType == COMPARTMENT_INTERIOR:
            if self.customrubberBand is None:
                self.customrubberBand = QtGui.QRubberBand(
            ...
            self.customrubberBand.setGeometry(QtCore.QRect(QtCore.QPoint(
                x0, y0), QtCore.QSize(abs(displacement.x()), abs(displacement.y()))))
    ...
    def editorMouseReleaseEvent(self, event):
        ...
        if clickedItemType == ITEM:
            if not self.state["move"]["happened"]:
                ...
                if isinstance(self.state["release"][
                              "item"], KineticsDisplayItem):
                    if not moose.element(
                            self.state["press"]["item"].mobj) == moose.element(
                            self.state["release"]["item"].mobj):
                        self.populate_srcdes(
                            self.state["press"]["item"].mobj,
                            self.state["release"]["item"].mobj)
                    else:
                        pass
        ...
        if clickedItemType == CONNECTOR:
            ...
            if actionType == "delete":
                ...
                if isinstance(element, moose.PoolBase):
                    self.graph = moose.element(
                        self.modelRoot + '/data/graph_0')
                    tablePath = utils.create_table_path(moose.element(
                        self.modelRoot), self.graph, element, "Conc")
                    table = utils.create_table(
                        tablePath, element, "Conc", "Table2")
                    self.layoutPt.plugin.view.getCentralWidget().plotWidgetContainer.plotAllData()
                    reply = QtGui.QMessageBox.information(
                        self, "plot Object", "Plot is added to Graph1", QtGui.QMessageBox.Ok)
                ...
                if self.state["move"]["happened"]:
                    ...
                    if isinstance(itemAtView, ComptItem):
                        # Solver should be deleted
                            ...
                            # or if copy has to made then oject should be in
                            # unZombify mode
                        deleteSolver(self.modelRoot)
                        lKey = [
                            key for key,
                            value in self.layoutPt.qGraCompt.items() if value == itemAtView][0]
                        iR = 0
                        iP = 0
                        ...
                        if itemAtView is None:
                            ...
                            srcdesString = (
                                (self.state["release"]["item"]).mobj).className
                            QtGui.QMessageBox.information(
                                None,
                                'Dropping Not possible',
                                'Dropping on \'{srcdesString}\' not allowed'.format(
                                    srcdesString=srcdesString),
                                QtGui.QMessageBox.Ok)
        ...
        if clickedItemType == COMPARTMENT_INTERIOR:
            if self.state["move"]["happened"]:
                ...
                self.selectedItems = selectedItems = self.items(
                    x0, y0, abs(displacement.x()), abs(displacement.y()))
                ...
                for item in selectedItems:
                    if isinstance(
                            item,
                            KineticsDisplayItem) and not isinstance(
                            item,
                            ComptItem):
                ...
                popupmenu.addAction(
                    "Delete", lambda: self.deleteSelections(
                        x0, y0, x1, y1))
                popupmenu.addAction(
                    "Zoom", lambda: self.zoomSelections(
                        x0, y0, x1, y1))
    ...
    def showConnector(self, item):
        ...
        for l in list(self.connectorlist.keys()):
            ...
            if isinstance(item.mobj, PoolBase) or isinstance(
                    item.mobj, ReacBase):
                if l == "clone":
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.state["press"]["mode"] == CONNECTION:
            ...
            des = [
                j for j in destination if isinstance(
                    j, KineticsDisplayItem)]
            if len(des):
    ...
    def updateItemTransformationMode(self, on):
        for v in list(self.sceneContainerPt.items()):
            ...
            if(not isinstance(v, ComptItem)):
                # if ( isinstance(v, PoolItem) or isinstance(v, ReacItem) or
                # isinstance(v, EnzItem) or isinstance(v, CplxItem) ):
                if isinstance(v, KineticsDisplayItem):
                    v.setFlag(
                        QtGui.QGraphicsItem.ItemIgnoresTransformations, on)

    def keyPressEvent(self, event):
        key = event.key()
        self.removeConnector()
        if (key == Qt.Qt.Key_A and (event.modifiers() & Qt.Qt.ShiftModifier)
                ):  # 'A' fits the view to iconScale factor
        ...
        for item in list(self.sceneContainerPt.items()):
            if isinstance(item, KineticsDisplayItem):
                ...
                if isinstance(item, ReacItem) or isinstance(
                    ...
                    item.setGeometry(
                        item.gobj.boundingRect().width() / 2,
                        item.gobj.boundingRect().height(),
                        item.gobj.boundingRect().width(),
                        item.gobj.boundingRect().height())
    ...
    def deleteSelections(self, x0, y0, x1, y1):
        if(x1 - x0 > 0 and y1 - y0 > 0):
            self.rubberbandlist = self.sceneContainerPt.items(
                self.mapToScene(
                    QtCore.QRect(
                        x0,
                        y0,
                        x1 - x0,
                        y1 - y0)).boundingRect(),
                Qt.Qt.IntersectsItemShape)
            for unselectitem in self.rubberbandlist:
                if unselectitem.isSelected():
    ...
    def deleteObj(self, item):
        ...
        for item in (qgraphicsitem for qgraphicsitem in self.rubberbandlist):
            ...
            # then it will created problem at qgraphicalitem not having parent.
            # So first delete enz and then delete pool
            if isinstance(item, MMEnzItem) or isinstance(
                    item, EnzItem) or isinstance(item, CplxItem):
                self.deleteItem(item)
        for item in (qgraphicsitem for qgraphicsitem in self.rubberbandlist):
            if not (isinstance(item, MMEnzItem) or isinstance(
                    item, EnzItem) or isinstance(item, CplxItem)):
                if isinstance(item, PoolItem):
                    plot = moose.wildcardFind(
                        self.layoutPt.modelRoot + '/data/graph#/#')
                    for p in plot:
                        if len(p.neighbors['requestOut']):
                            if item.mobj.path == moose.element(
                                    p.neighbors['requestOut'][0]).path:
                                p.tick = -1
                                moose.delete(p)
                                self.layoutPt.plugin.view.getCentralWidget().plotWidgetContainer.plotAllData()
                self.deleteItem(item)
        ...
        if len(object2lineInfo) == 1:
            for polygon, objdes, endtype, numL in object2lineInfo:
                if polygon == qpolygonline and objdes == src and endtype == endt:
                    ...
                    print(
                        " check this condition when is len is single and else condition",
                        qpolygonline,
                        objdes,
                        endtype)
        else:
    ...
    def deleteConnection(self, item):
        # Delete moose connection, i.e one can click on connection arrow and
        ...
        if isinstance(item, QtGui.QGraphicsPolygonItem):
            ...
            try:
                ...
                pass
            srcZero = [k for k, v in self.layoutPt.mooseId_GObj.items() if v == src[
                0]]
            srcOne = [k for k, v in self.layoutPt.mooseId_GObj.items() if v == src[
                1]]
            ...
            if isinstance(moose.element(srcZero[0]), moose.MMenz):
                ...
                # if ReadKKit get the msg from parent Pool, else from MMenz
                # itself.
                ...
                for msg in srcZero[0].msgIn:
                    if moose.element(msg.e1.path) == moose.element(
                            srcOne[0].path):
                        if src[2] == "t":
                            ...
                            self.sceneContainerPt.removeItem(item)
                            setupItem(
                                self.modelRoot, self.layoutPt.srcdesConnection)
                for msg in moose.element(srcZero[0].parent).msgIn:
                    if moose.element(msg.e2.path) == moose.element(
                            srcZero[0].parent.path):
                        if src[2] == 't':
    ...
    def populate_srcdes(self, src, des):
        ...
        if (isinstance(moose.element(src), PoolBase) and ((isinstance(
                moose.element(des), ReacBase)) or isinstance(moose.element(des), EnzBase))):
            ...
            if not found:
                ...
                QtGui.QMessageBox.information(
                    None, 'Connection Not possible', '{srcdesString}'.format(
                        srcdesString=srcdesString), QtGui.QMessageBox.Ok)
            ...
            moose.connect(src, 'nOut', des.x[numVariables], 'input')
              ...
              isinstance(moose.element(src), ZombieFunction) and (
            ...
            if ((element(des).parent).className != 'Enz'):
                ...
                if not found:
                    ...
                    srcdesString = '\"' + \
                        moose.element(
                            src).name + '\" is already connected to \"' + moose.element(des).name + '\" \n'
                    QtGui.QMessageBox.information(
                        None, 'Connection Not possible', '{srcdesString}'.format(
                            srcdesString=srcdesString), QtGui.QMessageBox.Ok)
                ...
                QtGui.QMessageBox.information(
                    ...
                    '\'{srcdesString}\' not allowed to connect'.format(
                        srcdesString=srcdesString),
                    QtGui.QMessageBox.Ok)
                callsetupItem = False
            ...
            if not found:
                ...
                srcdesString = desClass + ' is already connected as ' + '\'Substrate\'' + ' to ' + \
                    srcClass + ' \n \nIf you wish to connect this object then first delete the exist connection'
                QtGui.QMessageBox.information(
                    None, 'Connection Not possible', '{srcdesString}'.format(
                        srcdesString=srcdesString), QtGui.QMessageBox.Ok)
