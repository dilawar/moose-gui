...
class KineticsDisplayItem(QGraphicsWidget):
    ...
    defaultFontSize = 10
    qgtextItemSelectedChange = QtCore.pyqtSignal('PyQt_PyObject')
    ...
    def paint(self, painter=None, option=None, widget=None):
        ...
        if self.hasFocus() or self.isSelected():
            painter.setPen(
                QtGui.QPen(QtCore.Qt.black, 1.8, Qt.Qt.DashLine,
                           Qt.Qt.RoundCap, Qt.Qt.RoundJoin))
            painter.drawRect(self.boundingRect())

    # def mouseDoubleClickEvent(self, event):
    #     self.emit(QtCore.SIGNAL("qgtextDoubleClick(PyQt_PyObject)"),moose.element(self.mobj))
...
class GRPItem(QGraphicsRectItem):
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange:
            self.qgtextItemSelectedChange.emit(self.mobj)
            #  self.grpEmitter.emit(
...
class ComptItem(QGraphicsRectItem):
    ...
    # def mouseDoubleClickEvent(self, event):
    #     self.cmptEmitter.emit(QtCore.SIGNAL("qgtextDoubleClick(PyQt_PyObject)"),moose.element(self.mooseObj_))
